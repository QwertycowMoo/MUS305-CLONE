...
if __name__ == "__main__":
    # Addon TODO: Create randomize all parameters and make composition, have command line args
    ...
    has_pickup = args.pickup

    # Rainbow Connection
    # melody = [(64, 1), (67, 1), (72, 1),
    #           (74, .75), (76, .25), (76, 2),
    #           (65, 1), (67, 1.75), (72, .25),
    #           (72, 1), (74, 2),
    #           (64, 1), (67, 1.5), (71, 0.5),
    ...
    scale = 0
    if args.random:
        scale = musx.pick(MAJOR_SCALE_INTERVALS,
                          MINOR_SCALE_INTERVALS, LYDIAN_SCALE_INTERVALS)
    elif args.scale == 0:
        scale = MAJOR_SCALE_INTERVALS
    elif args.scale == 1:
        scale = MINOR_SCALE_INTERVALS
    elif args.scale == 2:
        scale - LYDIAN_SCALE_INTERVALS
    ### A SECTION ###
    print("A Section")
    chord_scale = create_chord_scale(num_sharps, scale)
    melody_length = 0
    for note in melody:
    ...
    for measure in split_melody(melody, time_sig):
        ...
        chords.append(harmonize(measure, chord_scale,
                      harm_interest=args.a_harm))
    # Give root position chord to sparkle generator
    countermelody = create_countermelody(
        chords, args.a_cmelnum, num_sharps, scale, time_sig)
    print(countermelody)
    inversions = voicelead_chords(chords, voicelead_rand=args.a_vlead)
    ...
    if args.b_len != -1:
        b_melody_length = args.b_len
    b_melody = create_b_melody(
        melody, b_melody_length, similarity=args.b_sim, slowdown=args.b_slow, transposition=args.b_trans)
    b_sharps = num_sharps - 1
    if args.b_key != 100:
        # just a flag value thats not within [-6, 7]
        b_sharps = args.b_key

    b_chord_scale = create_chord_scale(b_sharps, scale)
    b_chords = []
    for measure in split_melody(b_melody, time_sig):
        b_chords.append(harmonize(measure, b_chord_scale,
                        harm_interest=args.b_harm))
    b_countermelody = create_countermelody(
        chords, args.b_cmelnum, num_sharps, scale, time_sig)
    b_inversions = voicelead_chords(b_chords, voicelead_rand=args.b_vlead)
    b_chords = invert_chords(b_chords, b_inversions,
                             transpose_key=args.b_trans, lokey=args.b_lo, hikey=args.b_hi)
    ...
    for measure in split_melody(a_prime_melody, time_sig):
        a_prime_chords.append(
            harmonize(measure, chord_scale, harm_interest=args.ap_harm))
    a_prime_countermelody = create_countermelody(
        chords, args.ap_cmelnum, num_sharps, scale, time_sig)
    a_prime_inversions = voicelead_chords(
        a_prime_chords, voicelead_rand=args.ap_vlead)
    ...
    if has_pickup:
        start_time = (time_sig[0] * 4 /
                      time_sig[1]) * (60 / tempo)
    score.compose([[0, melody_composer(score, melody, tempo, chan=args.a_melchan)],
                   [start_time, chordal_composer(
                       score, chords, inversions, tempo, time_sig, rhy_interest=args.a_rhy, chan=args.a_harmchan)],
                   [start_time, bass_composer(
                       score, chords, tempo, time_sig, chan=5)],
                   [start_time, melody_composer(
                       score, countermelody, tempo, lokey=args.a_cmelhi, hikey=args.a_cmello, chan=args.a_cntchan)],

                   [b_start_time, melody_composer(
                       score, b_melody, tempo, chan=args.b_melchan)],
                   [b_start_time, chordal_composer(
                       score, b_chords, inversions, tempo, time_sig, rhy_interest=args.b_rhy, chan=args.b_harmchan)],
                   [b_start_time, bass_composer(
                       score, b_chords, tempo, time_sig, chan=5)],
                   [b_start_time, melody_composer(
                       score, b_countermelody, tempo, lokey=args.b_cmelhi, hikey=args.b_cmello, chan=args.b_cntchan)],

                   [a_prime_start_time, melody_composer(
                       score, a_prime_melody, tempo, chan=args.ap_melchan)],
                   [a_prime_start_time, chordal_composer(
                       score, a_prime_chords, inversions, tempo, time_sig, rhy_interest=args.ap_rhy, chan=args.ap_harmchan)],
                   [a_prime_start_time, bass_composer(
                       score, a_prime_chords, tempo, time_sig, chan=5)],
                   [a_prime_start_time, melody_composer(
                       score, a_prime_countermelody, tempo, lokey=args.b_cmelhi, hikey=args.ap_cmello, chan=args.ap_cntchan)],
                   ])
    pp.pprint(chords)
    pp.pprint(b_chords)
